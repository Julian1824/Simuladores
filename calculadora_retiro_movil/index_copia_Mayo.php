...
var fiestas=new Array(new Array(1,1),new Array(10,1),new Array(21,3),new Array(14,4),new Array(15,4),new Array(1,5),new Array(30,5),new Array(20,6),new Array(27,6),new Array(4,7),new Array(20,7),new Array(7,8),new Array(15,8),new Array(17,10),new Array(7,11),new Array(14,11),new Array(8,12),new Array(21,12));


///////nuevo metodo***********************************************************************
diastotales=0;
var fechafindefinitiva = new Date(year, mesfinal-1, diafinal);  
var fechaInicio = new Date('2022-'+mesfinal+'-16');
...
console.log(fechafindefinitiva.getFullYear() + '/' + (fechafindefinitiva.getMonth() + 1) + '/' + fechafindefinitiva.getDate());
fechafindefinitiva.setDate(fechafindefinitiva.getDate() + 1);
}

var fechafindefinitiva = new Date(year, mesfinal-1, diafinal);  
var fechaInicio = new Date('2022-04-15');
while(fechafindefinitiva.getTime() <= fechaInicio.getTime()){
for (var i=0;i<fiestas.length; i++) {
    fechabuscar=fiestas[i];
    fechacomparar=parseInt(fechafindefinitiva.getDate())+","+parseInt(fechafindefinitiva.getMonth()+1);
  //alert(fechabuscar);
    //alert(fechacomparar);
...
//validar si es fecha de corte es feriado
var fechafindefinitiva = new Date(year, mesfinal-1, diafinal);  
for (var i=0;i<fiestas.length; i++) {
    fechabuscar=fiestas[i];
    fechacomparar=parseInt(15)+","+parseInt(4);
  //alert(fechabuscar);
    //alert(fechacomparar);
    ...
    if (ciclo==<?php echo $i ?>)
{
   ...
   //alert("dias dif"+diasdiferencia);

    if(ciclo==1)
    {
        ////Buscar el ultimo dia del mes seleccionado
        diasdiferencia=(31-3);
    }
    if(ciclo==2)
    {////Buscar el ultimo dia del mes seleccionado
...
document.getElementById("fechafinalfactura").innerHTML=mesfactura+"-"+year;
    ...
    document.getElementById("mensaje").innerHTML="Solicitud de retiro debe hacerse tres días habiles antes del corte del ciclo de consumo.";
}
}
...
if(ciclo==30){
document.getElementById("mensaje").innerHTML="Ciclo 1	*Consumo del 1 al 30";
}
if(ciclo==1){document.getElementById("mensaje").innerHTML="Ciclo	2	*Consumo del 2 al 1";
}
if(ciclo==2){
...
window.holidays = $.ajax({ dataType: 'json', url: 'holiday.json', async: false }).responseJSON;
